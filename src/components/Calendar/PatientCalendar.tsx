...
import { createSchedule, deleteSchedule, fetchPatientSchedules, fetchPatients } from '../../context/AuthProvider/util';
...
const PatientCalendar: React.FC<PatientCalendarProps> = ({ patientId }) => {
    ...
    useEffect(() => {
        // Aqui você pode fazer uma chamada à API ou acessar seu banco de dados para obter os agendamentos do paciente com base no patientId
        // Vamos supor que você tenha uma função getPatientAppointments que retorna uma Promise com os agendamentos

        const fetchSchedules = async () => {
            try {
                const schedulesData: Schedule[] = await fetchPatientSchedules(token, patientId);
                setSchedules(schedulesData);
            } catch (error) {
                console.log('Erro ao obter os agendamentos do paciente:', error);
            }
        };
